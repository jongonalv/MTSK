...
const EquiposButtons = ({ setIsModalOpen, equipo, onEquiposUpdated, reloadEquipos, fetchEquipos }) => {
  ...
  const handleDelete = async (equipo) => {
    ...
    if (confirmDelete) {
      try {
        const response = await fetch(`${process.env.REACT_APP_API_URL}/api/equipo/${equipo.etiquetaEquipo}`, {
          method: 'DELETE',
        });
  ...
  const handleAssign = async (equipo, usuario) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL}/api/equipo/${equipo.etiquetaEquipo}/asignar`, {
        method: 'POST',
        headers: {
